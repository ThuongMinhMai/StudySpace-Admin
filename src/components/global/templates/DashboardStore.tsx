...
import { Bus, Ticket, Route, HandCoins,DoorOpen  } from 'lucide-react'
...
export default function DashboardStore() {
  ...
  return (
    <div className='flex flex-col gap-5  w-full'>
      ...
      <section className='grid w-full grid-cols-1 gap-10 gap-x-8 transition-all sm:grid-cols-2 xl:grid-cols-3'>
        <Card key={1} amount={data ? formatPrice(data.totalIncome) : '0'} description='' icon={HandCoins} label='Doanh thu cả năm' color='red'/>
        <Card key={2} amount={data ? data.totalBooking.toString() : '0'} description='' icon={Ticket} label='Tổng số lượt đặt phòng' color='orange'/>
        <Card key={3} amount={data ? data.totalRoom.toString() : '0'} description='' icon={DoorOpen } label='Tổng số phòng' color='blue'/>
      </section>
