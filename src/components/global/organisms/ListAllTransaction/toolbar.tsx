...
export function DataTableToolbar<TData>({ table }: { table: Table<TData> }) {
  ...
  const uniqueType = Array.from(table.getColumn('type')?.getFacetedUniqueValues()?.entries() || []).map(([key]) => key)
  ...
  return (
    <div className='ml-2 mb-2 flex justify-between'>
      <div className='flex space-x-2 '>
        ...
        <DataTableFacetedFilter column={table.getColumn('type')} title='Loại giao dịch' options={uniqueType} />
