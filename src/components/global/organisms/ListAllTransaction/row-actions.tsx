...
export function DataTableRowActions<TData extends Transaction>({ row,handleStatusChange }: DataTableRowActionsProps<TData>) {
  const { user } = useAuth()
  ...
  const [tempStatus, setTempStatus] = useState<{ [key: string]: string }>({
    [row.original.id]: row.original.status
  })
